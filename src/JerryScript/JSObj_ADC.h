...
public:
    ...
    static void Register()
    {
        JerryScript::UseThenFree(jerry_function_external(OnConstructed), [](auto jsFnObj){
            JerryScript::SetGlobalPropertyNoFree("ADC", jsFnObj);
...
private:
                                     ...
                                     const jerry_length_t     argc)
    {
        ...
        ADC *obj = (ADC *)JerryScript::GetNativePointer(callInfo->this_value, &typeInfo_);
        ...
        else
        {
            ...
            if (fnName == "ReadVolts")
            {
                double val = (double)obj->GetMilliVolts() / 1'000;

                retVal = jerry_number(val);
            }
            else if (fnName == "ReadRaw")
            {
                retVal = jerry_number(obj->Read());
            }
        }
