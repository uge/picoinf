...
public:
    ...
    static void Register()
    {
        JerryScript::UseThenFree(jerry_function_external(OnConstructed), [](auto jsFnObj){
            JerryScript::SetGlobalPropertyNoFree("BMP280", jsFnObj);
...
private:
                                        ...
                                        const jerry_length_t     argc)
    {
        ...
        BMP280 *obj = (BMP280 *)JerryScript::GetNativePointer(callInfo->this_value, &typeInfo_);
                                       ...
                                       const jerry_length_t     argc)
    {
        ...
        else
        {
            ...
            {
                string error = string{"BMP280 I2C address ("} + ToHex(addr) + ") is invalid";
                retVal = jerry_throw_sz(JERRY_ERROR_COMMON, error.c_str());
            }
            else
            {
                // Create a new object
                BMP280 *obj = new BMP280(addr, instance_);
                ...
                {
                    string error = string{"BMP280 I2C address ("} + ToHex(addr) + ") is unresponsive";
                    retVal = jerry_throw_sz(JERRY_ERROR_COMMON, error.c_str());
