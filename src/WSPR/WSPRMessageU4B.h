...
private:
    ...
    static string EncodeCallsign(string id13, string grid56, uint32_t altM)
    {
        ...
        // Log("grid5Val(", grid5Val, "), grid6Val(", grid6Val, "), altFracM(", altFracM, "), ");
        ...
        // Log("id6Val(", id6Val, "), id5Val(", id5Val, "), id4Val(", id4Val, "), id2Val(", id2Val, "), ");
...
public:
    ...
    {
        // inputs match the way the web encode/decode works
        Shell::AddCommand("wspr.u4b.encode", [](vector<string> argList){
            string   id13       = argList[0];
            string   grid6      = argList[1];
            string   grid56     = grid6.substr(4);
            uint32_t altM       = atol(argList[2].c_str());
            int8_t   tempC      = atoi(argList[3].c_str());
            double   voltage    = atof(argList[4].c_str());
            uint8_t  speedKnots = atoi(argList[5].c_str());
            bool     gpsValid   = argList[6] == "0" ? false : true;
            ...
            msg.SetGpsValid(gpsValid);

            string  callsign = msg.GetCallsign();
            string  grid     = msg.GetGrid();
            uint8_t powerDbm = msg.GetPowerDbm();

            Log("Encoding");
            Log("id13(", id13, "), grid6(", grid6, "), altM(", altM, "), tempC(", tempC, "), voltage(", voltage, "), speedKnots(", speedKnots, "), gpsValid(", gpsValid, ")");
            LogNL();
            Log(callsign, " ", grid, " ", powerDbm);
            LogNL();
        }, { .argCount = 7, .help = "id13, grid6, altM, tempC, voltage, speedKnots, gpsValid"});
    }
};
