...
public:
    ...
    {
        Shell::AddCommand("wspr.mins", [](vector<string> argList){
            for (const auto &bandData : bandDataList_)
            {
                Log(bandData.band, ",\t minList: ", GetMinuteListForBand(bandData.band));
            }
        }, { .argCount = 0, .help = "see minute list by band"});

        Shell::AddCommand("wspr.chan", [](vector<string> argList){
            string band = argList[0];
            uint16_t channel = atoi(argList[1].c_str());

            ChannelDetails cd = GetChannelDetails(band, channel);

            Log("Details for band ", band, ", channel ", channel);
            Log("id13: ", cd.id13);
            Log("min : ", cd.min);
            Log("lane: ", cd.lane);
            Log("freq: ", Commas(cd.freq));
        }, { .argCount = 2, .help = "see data by <band> and <channel>"});
    }
};
