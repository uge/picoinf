...
static const device *DEV_UART_0 = DEVICE_DT_GET(DT_NODELABEL(uart0));
static const device *DEV_UART_1 = DEVICE_DT_GET(DT_NODELABEL(uart1));
static const device *DEV_UART_USB = DEVICE_DT_GET(DT_NODELABEL(cdc_acm0));

const device *UartDeviceMap(UART uart)
{
    const device *dev = DEV_UART_0;

    if      (uart == UART::UART_1)   { dev = DEV_UART_1;   }
    else if (uart == UART::UART_USB) { dev = DEV_UART_USB; }
...
static const device *UartGetDevice()
{
    const device *dev = DEV_UART_0;
...
static void UartInputIrqHandler(const struct device *dev, void * /* user_data */)
{
    KSemaphore      *sem = nullptr;
    vector<uint8_t> *vec = nullptr;
    DataStreamDistributor *dist = nullptr;

    if (dev == DEV_UART_1)
    {
        sem = &UART_1_INPUT_SEM;
        vec = &UART_1_INPUT_VEC;
        dist = &UART_1_INPUT_DATA_STREAM_DISTRIBUTOR;
    }
    else if (dev == DEV_UART_USB)
    {
        sem = &UART_USB_INPUT_SEM;
        vec = &UART_USB_INPUT_VEC;
        dist = &UART_USB_INPUT_DATA_STREAM_DISTRIBUTOR;
    }

    while (uart_irq_update(dev) && uart_irq_is_pending(dev))
    {
        // "uart_fifo_read must be called until it reads all available data in the FIFO
        // (i.e. until it returns less data than was requested)."
        if (uart_irq_rx_ready(dev))
        {
            int recvLen = uart_fifo_read(dev, READ_BUF, READ_BUF_SIZE);

            // if you got data, and there's a place to put it and someone wants it
            if (dist->GetCallbackCount() == 0)
            {
                vec->clear();
            }
            else if (recvLen > 0 && vec->size() < UART_INPUT_PIPE_SIZE)
            {
                // calculate how many bytes will fit
                int bytesToCopy = min((size_t)UART_INPUT_PIPE_SIZE - vec->size(), (size_t)recvLen);
...
////////////////////////////////////////////////////////////////////////////////
// Threads dedicated to cranking out uart at low priority
// 
// https://docs.zephyrproject.org/3.0.0/reference/peripherals/uart.html
//
// It is wasteful to have so much stack space for all the threads, could
// consolidate down to one.  Would require:
// - use k_poll
//   - change pipe to be fifo (only fifo is compatible, not pipe)
// - change uart_poll_out to use interrupts
//   - as seen in cdc_acm example
//
// For now this is "fine" since I'm sick of dealing with it.
//
////////////////////////////////////////////////////////////////////////////////
...
//
// UART 0
// (input handled by shell)
//
static void ThreadFnUART0Output()
{
    while (true)
    {
        char c;
        if (UART_0_OUTPUT_PIPE.Get(c, K_FOREVER))
        {
            uart_poll_out(DEV_UART_0, c);
        }
    }
}
K_THREAD_DEFINE(ThreadUART0Output, STACKSIZE, ThreadFnUART0Output, NULL, NULL, NULL, PRIORITY, 0, 0);
...
static void ThreadFnUART1Input()
{
    while (true)
    {
        if (UART_1_INPUT_SEM.Take(K_FOREVER))
        {
            ...
            UART_1_INPUT_DATA_STREAM_DISTRIBUTOR.Distribute(UART_1_INPUT_VEC);
            ...
            UART_1_INPUT_SEM.Reset();
        }
    }
}
K_THREAD_DEFINE(ThreadUART1Input, STACKSIZE, ThreadFnUART1Input, NULL, NULL, NULL, PRIORITY, 0, 0);

static void ThreadFnUART1Output()
{
    while (true)
    {
        char c;
        if (UART_1_OUTPUT_PIPE.Get(c, K_FOREVER))
        {
            uart_poll_out(DEV_UART_1, c);
        }
    }
}
K_THREAD_DEFINE(ThreadUART1Output, STACKSIZE, ThreadFnUART1Output, NULL, NULL, NULL, PRIORITY, 0, 0);
...
void UartOut(char c)
{
    const device *dev = UartGetDevice();
...
static bool uart1Enabled = true;
void UartDisable(UART uart)
{
    // prevent ISRs from firing, or calling this code
    IrqLock lock;

    // clobber any queued serial data inbound or outbound
    UartClearTxBuffer(uart);
    UartClearRxBuffer(uart);

    if (uart == UART::UART_1)
    {
        uart_irq_rx_disable(DEV_UART_1);
...
uint32_t UartGetBaud(UART uart)
{
    ...
    uart_config cfg;
    int ret = uart_config_get(UartDeviceMap(uart), &cfg);
...
int UartInit()
{
    ...
    uart_irq_callback_set(DEV_UART_1, UartInputIrqHandler);
    // Enable UART1 the same way as when it becomes re-enabled once disabled
    UartEnable(UART::UART_1);

    // The USB serial interface should always exist.
    // It can be used for one of two things:
    // - Application utilization (arbitrary style)
    // - Zephyr shell
    //
    // If the shell, must not set up interrupts to code here, or the shell will
    // never see input.
    // Also, the standard output of the application is directed to the usb
...
SYS_INIT(UartInit, APPLICATION, 12);
SYS_INIT(UartSetupShell, APPLICATION, 80);
